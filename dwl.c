...
static uint32_t draw_text(pixman_image_t *pix, int x, int y, unsigned int w,
                          unsigned int h, unsigned int lpad, const char *text,
                          pixman_color_t *clr);
...
bool bar_accepts_input(struct wlr_scene_buffer *buffer, double *sx,
                       double *sy) {
  return true;
}
...
bool buffer_begin_data_ptr_access(struct wlr_buffer *wlr_buffer, uint32_t flags,
...
void buttonpress(struct wl_listener *listener, void *data) {
  ...
  if ((node = wlr_scene_node_at(&layers[LyrBottom]->node, cursor->x, cursor->y,
                                NULL, NULL)) &&
  ...
  for (const char *p = text; *p; p++) {
    ...
    if (render) {
      ...
      pixman_image_composite32(PIXMAN_OP_OVER, clr_pix, glyph->pix, pix, 0, 0,
                               0, 0, x + glyph->x, ty - glyph->y, glyph->width,
                               glyph->height);
    }
...
static void draw_rect(pixman_image_t *pix, int16_t x, int16_t y, uint16_t w,
                      uint16_t h, int filled, pixman_color_t *bg) {
  /*
  ...
  if (filled)
    ...
    pixman_image_fill_rectangles(
        ...
        (pixman_rectangle16_t[4]){{x, y, w, 1},
                                  {x, y + h - 1, w, 1},
                                  {x, y, 1, h},
                                  {x + w - 1, y, 1, h}});
}
...
void drawbar(Monitor *mon) {
  ...
  if (mon == selmon) {
    ...
    draw_text(pix, borderpx + mw - tw, y, tw + borderpx, mh, 0, stext,
              &normbarfg);
  }
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    draw_rect(pix, x + boxs, y + boxs, boxw, boxw, sel,
              urg & 1 << i ? &selbarbg : (sel ? &selbarfg : &normbarfg));
    x += w;
  }
  ...
  wlr_scene_node_set_position(&mon->scene_buffer->node, mon->m.x + sp,
                              mon->m.y + vp +
                                  (topbar ? 0 : mon->m.height - mon->b.height));
...
void handlecursoractivity(bool restore_focus) {
...
void monocle(Monitor *m) {
  ...
  wl_list_for_each(c, &clients, link) {
    ...
    if (!monoclegaps)
      ...
      resize(c,
             (struct wlr_box){.x = m->w.x + gappoh,
                              .y = m->w.y + gappov,
                              .width = m->w.width - 2 * gappoh,
...
void setup(void) {
  ...
  LISTEN(&vkb_group.wlr_group->keyboard.events.modifiers, &vkb_group.modifiers,
         keypressmod);
...
int status_in(int fd, unsigned int mask, void *data) {
  char status[1024];
  ssize_t n;
  ...
  wl_list_for_each(c, &clients, link) {
    ...
    if (i < m->nmaster) {
      if (n == 1 && center && c->centered) {
        resize(c,
               (struct wlr_box){.x = m->w.width / 4,
                                .y = m->w.y,
                                .width = m->w.width / 2,
                                .height = m->w.height - 2 * c->bw},
               0);
        return;
      }
      r = MIN(n, m->nmaster) - i;
...
void togglecenter(const Arg *arg) {
  center = !center;
  tile(selmon);
}
